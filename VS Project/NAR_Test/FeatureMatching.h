...
public:
	...
	~FeatureMatching();
	bool track(cv::Mat &, bool& markerFound, std::vector<cv::Point2f> &matchedCorners, std::vector<cv::Point2f>& trackedCorners, std::vector<cv::Point2f>& tracks, int& tracksCount, cv::Mat& frameGray, cv::Mat maskBounding);
